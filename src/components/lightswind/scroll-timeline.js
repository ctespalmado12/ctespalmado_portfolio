...
const DEFAULT_EVENTS = [
    ...
    },
    {
        ...
        subtitle: "Organization Name",
        bullets: [
            "Details about this significant milestone and its impact.",
        ]
    },
    {
        year: "2021",
        ...
        subtitle: "Organization Name",
        bullets: [
            "Information about this key event in the timeline.",
        ]
    },
];
export const ScrollTimeline = ({ events = DEFAULT_EVENTS, title = "Timeline", subtitle = "Scroll to explore the journey", animationOrder = "sequential", cardAlignment = "alternating", lineColor = "bg-primary/30", activeColor = "bg-primary", progressIndicator = true, cardVariant = "default", cardEffect = "none", parallaxIntensity = 0.2, progressLineWidth = 2, progressLineCap = "round", dateFormat = "badge", revealAnimation = "fade", className = "", connectorStyle = "dashed", perspective = false, darkMode = false, smoothScroll = true, }) => {
    ...
    const getConnectorClasses = () => {
        const baseClasses = cn("absolute left-1/2 transform -translate-x-1/2", lineColor || "bg-slate-400/30" );
        ...
        switch (connectorStyle) {
            ...
            case "dashed":
                ...
                baseClasses,
                "bg-transparent",
                "border-l border-dashed border-slate-400/30",
                `[border-left-width:${progressLineWidth}px]`
                );
            case "line":
            default:
                                        ...
                                        : cardAlignment === "left"
                                            ...
                                            : "lg:flex-row-reverse lg:justify-start"), children: [_jsx("div", { className: cn("absolute top-1/2 transform -translate-y-1/2 z-30", "left-1/2 -translate-x-1/2"), children: _jsx(motion.div, { className: cn("w-6 h-6 rounded-full border-4 bg-background flex items-center justify-center", index <= activeIndex
                                                    ...
                                                    ease: "easeInOut",
                                                                        ...
                                                                        ? `text-${event.color}`
                                                                        : "text-primary"), children: event.year })] })) : (_jsx("p", { className: "text-lg font-bold text-primary mb-2", children: event.year })), _jsx("h3", { className: "text-xl font-bold mb-1", children: event.title }), event.subtitle && (_jsx("p", { className: "text-muted-foreground font-medium mb-2", children: event.subtitle })), _jsxs("ul", { className: "list-none space-y-1", children: event.bullets?.map((b, i) => _jsxs("li", { className:"flex items-start gap-2 text-[15px] md:text-[17px] lg:text-xl", children: [_jsx(Check, { className: "mt-1 w-4 h-4 shrink-0 text-[#21ba45]", "aria-hidden": "true" }), _jsx("span", {children: b})]}, i)) })] }) }) })] }, event.id || index));
                            }) })] }) })] }));
};
